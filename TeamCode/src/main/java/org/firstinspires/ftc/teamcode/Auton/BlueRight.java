    ...
    public void loop() {
        ...
        switch (_state) {
            ...
            case back:
                if(_justEntered){
                    _justEntered=false;
                    Robot.getDrivetrain().runDistance(0.2, 24, _Drivetrain.Movements.backward);
                }
                else if(!Robot.getDrivetrain().isBusy()){
            ...
            case utilizeopencv:
                if (_justEntered) {
                    ...
                    if (_color.equals("Blue")) {
                        Robot.getDrivetrain().runDistance(.3, 50, _Drivetrain.Movements.right);
                    } else if (_color.equals("Green")) {

                        Robot.getDrivetrain().runDistance(.55, 25, _Drivetrain.Movements.right);
                    } else {
                        Robot.getDrivetrain().runDistance(.55, 8, _Drivetrain.Movements.right);
                    }
                }
